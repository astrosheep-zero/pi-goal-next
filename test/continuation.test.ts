...
function harness(g = goal()) {
  ...
  return { deps, sent, commits, setIdle: (v: boolean) => { idle = v; }, setPending: (v: boolean) => { pending = v; }, setResult: (v: any) => { nextResult = v; }, blockCommit: () => { blocked = true; return () => { blocked = false; release?.(); }; } };
}
